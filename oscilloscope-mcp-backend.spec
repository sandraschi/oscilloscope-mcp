...
# PyInstaller spec — produce oscilloscope-mcp-backend.exe for Tauri NSIS embedding.
# Fleet standard: strip=False, upx=False, noarchive=True (see tauri_nsis_building.md).
# Usage (from repo root):
#   uv run pyinstaller oscilloscope-mcp-backend.spec --distpath dist --clean --noconfirm

block_cipher = None

a = Analysis(
    ["src/oscilloscope_mcp/__main__.py"],
    pathex=["src"],
    binaries=[],
    datas=[("src/oscilloscope_mcp", "oscilloscope_mcp")],
    hiddenimports=[
        "uvicorn.logging",
        "uvicorn.loops",
